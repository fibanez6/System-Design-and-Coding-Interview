...
[example]
====
.Example of CAP theorem in practice
Consider a distributed database system like MongoDB deployed across multiple data centers. If a network partition occurs between the data centers (Partition Tolerance), the system must choose between:

- Consistency: Rejecting requests to ensure all nodes see the same data, but some requests may fail (sacrificing Availability).
- Availability: Allowing requests to succeed with potentially stale or divergent data (sacrificing Consistency).

For example, if a write is made in one data center during a partition, other data centers may not see the update immediately. MongoDB can be configured to prioritize either Consistency or Availability, but not both during a partition, illustrating the CAP theorem trade-offs.
====
